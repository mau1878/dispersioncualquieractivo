...
# Calculate the KDE
kde = stats.gaussian_kde(data['Dispersion_Percent'].dropna())
x_values = np.linspace(data['Dispersion_Percent'].min(), data['Dispersion_Percent'].max(), 1000)
...
fig_hist.add_trace(
    go.Scatter(
        x=x_values,
        y=kde_values * len(data['Dispersion_Percent'].dropna()) * (x_values[1] - x_values[0]),  # Scale KDE to match histogram
        mode='lines',
